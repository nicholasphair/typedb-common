...
def upload(url, username, password, local_fn, remote_fn):
    print(local_fn)
    upload_status_code = check_output([
        'curl', '--silent', '--output', '/dev/stderr', '--write-out', '%{http_code}', '-u',
        '{}:{}'.format(username, password), '--upload-file', local_fn,
        urljoin(url, remote_fn)
    ])
    print(upload_status_code)

    #if upload_status_code not in {'200', '201'}:
    #    raise Exception('upload of {} failed, got HTTP status code {}'.format(local_fn, upload_status_code))


def upload_orig(url, username, password, local_fn, remote_fn):
...
maven_repositories = {
    "snapshot": "https://repo.vaticle.com/repository/maven-snapshot/",
    "release": "https://repo.vaticle.com/repository/maven/"
}
maven_url = maven_repositories[repo_type]
jar_path = 'libtypedb-runner.jar'
pom_file_path = "assemble-maven_pom.xml"
srcjar_path = "libtypedb-runner-src.jar"
...
if repo_type not in [snapshot, release]:
    raise ValueError("Invalid repository type: {}. It should be one of these: {}".format(
        repo_type, [snapshot, release]))
if repo_type == 'snapshot' and len(re.findall(version_snapshot_regex, version)) == 0:
    raise ValueError('Invalid version: {}. An artifact uploaded to a {} repository '
                     'must have a version which complies to this regex: {}'.format(version, repo_type,
                                                                                   version_snapshot_regex))
if repo_type == 'release' and len(re.findall(version_release_regex, version)) == 0:
    raise ValueError('Invalid version: {}. An artifact uploaded to a {} repository '
                     'must have a version which complies to this regex: {}'.format(version, repo_type,
                                                                                   version_release_regex))

filename_base = '{coordinates}/{artifact}/{version}/{artifact}-{version}'.format(coordinates=group_id.text.replace(
    '.', '/'),
                                                                                 version=version,
...
if os.path.exists(srcjar_path):
    ...
    with tempfile.NamedTemporaryFile(mode='wt', delete=False) as srcjar_sha1:
